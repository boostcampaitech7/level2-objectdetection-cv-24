...
def kmeans_plus_plus_anchors(wh, n_anchors, iterations=30):
    wh = wh[wh[:, 0] > 0]  # 너비가 0보다 큰 것만 선택
    wh = wh[wh[:, 1] > 0]  # 높이가 0보다 큰 것만 선택
    wh = wh[np.logical_and(wh[:, 0] < 1, wh[:, 1] < 1)]  # 1보다 작은 것만 선택 (정규화된 경우)

    kmeans = KMeans(n_clusters=n_anchors, init='k-means++', n_init=10, max_iter=iterations)
    kmeans.fit(wh)
...
def main(ann_file, n_anchors, img_size):
    ...
    anchors = kmeans_plus_plus_anchors(wh, n_anchors)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate optimized anchors for YOLO using K-means++")
