...
class YOLOv5Head(BaseDenseHead):
    ...
    Args:
        ...
        anchor_generator(dict): Points generator feature maps in
    ...
    def __init__(self,
        ...
        self.in_channels = [int(channel) for channel in self.in_channels]
        self.widen_factor = widen_factor
        self.featmap_strides = featmap_strides
        self.num_levels = len(self.featmap_strides)
        self.num_out_attrib = 5 + num_classes
    ...
    def _init_layers(self):
        ...
        self.convs_pred = nn.ModuleList()
        for i in range(self.num_levels):
            conv_pred = nn.Conv2d(int(self.in_channels[i]),
                                  self.num_base_priors * self.num_out_attrib,
                                  1)
    ...
    def special_init(self):
        ...
        priors_base_sizes = torch.tensor(
            self.anchor_generator.base_sizes, dtype=torch.float)
    ...
    def predict_by_feat(self,
        ...
        results_list = []
        for idx, (bboxes, scores, objectness,
             img_meta) in enumerate(zip(flatten_decoded_bboxes, flatten_cls_scores,
    ...
    def loss_by_feat(
        ...
        for i in range(self.num_levels):
            ...
            loss_box_i, iou = self.loss_bbox(decoded_bbox_pred, bboxes_targets, return_iou=True)
            loss_box += loss_box_i
